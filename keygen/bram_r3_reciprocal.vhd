...
-- contains the memories for the reciprocal calculation of r3 polynomials
entity bram_r3_reciprocal is
  generic(
    bram_address_width : integer := integer(ceil(log2(real((p + 1)/2))));
...
architecture RTL of bram_r3_reciprocal is
  ...
  function offset_addr(addr: std_logic_vector) return std_logic_vector is
    variable result: std_logic_vector(addr'range);
  begin
    result := std_logic_vector(unsigned(addr) + to_unsigned(address_offset, addr'length));
    return result;
  end;
...
begin
  -- Apply offset to all read/write addresses
  bram_fg_one_address_b <= offset_addr(bram_f_address_b) when swap_mask_s = '0' else offset_addr(bram_g_address_b);
  bram_fg_two_address_b <= offset_addr(bram_g_address_b) when swap_mask_s = '0' else offset_addr(bram_f_address_b);
  bram_fg_one_address_a <= offset_addr(bram_f_address_a) when swap_mask_s = '0' else offset_addr(bram_g_address_a);
  bram_fg_two_address_a <= offset_addr(bram_g_address_a) when swap_mask_s = '0' else offset_addr(bram_f_address_a);

  bram_vr_one_address_b <= offset_addr(bram_v_address_b) when swap_mask_s = '0' else offset_addr(bram_r_address_b);
  bram_vr_two_address_b <= offset_addr(bram_r_address_b) when swap_mask_s = '0' else offset_addr(bram_v_address_b);
  bram_vr_one_address_a <= offset_addr(bram_v_address_a) when swap_mask_s = '0' else offset_addr(bram_r_address_a);
  bram_vr_two_address_a <= offset_addr(bram_r_address_a) when swap_mask_s = '0' else offset_addr(bram_v_address_a);

  -- Data and write enables remain the same
  ...
  -- Instantiate BRAMs (unchanged)
  block_ram_inst_fg_one : entity work.SDP_dist_RAM
    generic map(ADDRESS_WIDTH => bram_address_width, DATA_WIDTH => bram_data_width)
    port map(clock => clock, address_a => bram_fg_one_address_a, data_out_a => bram_fg_one_data_out_a,
             address_b => bram_fg_one_address_b, write_b => bram_fg_one_write_b, data_in_b => bram_fg_one_data_in_b);

  block_ram_inst_fg_two : entity work.SDP_dist_RAM
    generic map(ADDRESS_WIDTH => bram_address_width, DATA_WIDTH => bram_data_width)
    port map(clock => clock, address_a => bram_fg_two_address_a, data_out_a => bram_fg_two_data_out_a,
             address_b => bram_fg_two_address_b, write_b => bram_fg_two_write_b, data_in_b => bram_fg_two_data_in_b);

  block_ram_inst_vr_one : entity work.SDP_dist_RAM
    generic map(ADDRESS_WIDTH => bram_address_width, DATA_WIDTH => bram_data_width)
    port map(clock => clock, address_a => bram_vr_one_address_a, data_out_a => bram_vr_one_data_out_a,
             address_b => bram_vr_one_address_b, write_b => bram_vr_one_write_b, data_in_b => bram_vr_one_data_in_b);

  block_ram_inst_vr_two : entity work.SDP_dist_RAM
    generic map(ADDRESS_WIDTH => bram_address_width, DATA_WIDTH => bram_data_width)
    port map(clock => clock, address_a => bram_vr_two_address_a, data_out_a => bram_vr_two_data_out_a,
             address_b => bram_vr_two_address_b, write_b => bram_vr_two_write_b, data_in_b => bram_vr_two_data_in_b);
